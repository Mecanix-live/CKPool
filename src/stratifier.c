...
{
    char *msg, *msg_with_eol;
    size_t len;

    //msg = json_dumps(val, JSON_NO_UTF8 | JSON_PRESERVE_ORDER | JSON_COMPACT);
    msg = json_dumps(val, JSON_PRESERVE_ORDER | JSON_COMPACT);
    if (unlikely(!msg)) {
        LOGWARNING("Failed to create JSON dump in upstream_json");
        return;
    }

    len = strlen(msg);
    msg_with_eol = realloc(msg, len + 2);
    if (unlikely(!msg_with_eol)) {
        free(msg);
        LOGWARNING("Memory allocation failed in upstream_json");
        return;
    }
    msg = msg_with_eol;
    strcat(msg, "\n");
...
char *stratifier_stats(ckpool_t *ckp, void *data)
{
	...
	buf = json_dumps(val, JSON_PRESERVE_ORDER);
	json_decref(val);
...
void parse_upstream_reqtxns(ckpool_t *ckp, json_t *val)
{
    ...
    //msg = json_dumps(ret, JSON_NO_UTF8 | JSON_PRESERVE_ORDER | JSON_COMPACT);
    msg = json_dumps(ret, JSON_PRESERVE_ORDER | JSON_COMPACT);
    json_decref(ret);

    if (unlikely(!msg)) {
        LOGWARNING("Failed to create JSON dump in parse_upstream_reqtxns");
        return;
    }
    ...
    if (unlikely(!msg_with_eol)) {
        free(msg);
        LOGWARNING("Memory allocation failed in parse_upstream_reqtxns");
        return;
    }
    msg = msg_with_eol;
    strcat(msg, "\n");
...
static void upstream_auth(ckpool_t *ckp, stratum_instance_t *client, json_params_t *jp)
{
    ...
    if (unlikely(!msg)) {
        LOGWARNING("Failed to create JSON dump in upstream_auth");
        return;
    }
    ...
    if (unlikely(!msg_with_eol)) {
        free(msg);
        LOGWARNING("Memory allocation failed in upstream_auth");
        return;
    }
    msg = msg_with_eol;
    strcat(msg, "\n");
	...
	while (42) {
		...
		while ((user = next_user(sdata, user)) != NULL) {
			...
			if (!idle) {
				...
				s = json_dumps(val, JSON_PRESERVE_ORDER | JSON_COMPACT);
			...
			s = json_dumps(val, JSON_PRESERVE_ORDER | JSON_REAL_PRECISION(16) | JSON_INDENT(1));
			if (likely(s)) {
			    size_t len = strlen(s);
			    char *s_with_nl = realloc(s, len + 2);
			    if (likely(s_with_nl)) {
			        s = s_with_nl;
			        strcat(s, "\n");
			    } else {
			        LOGDEBUG("Failed to realloc for newline in statsupdate");
			    }
			}
		...
		s = json_dumps(val, JSON_PRESERVE_ORDER | JSON_REAL_PRECISION(3));
		json_decref(val);
		LOGNOTICE("Pool:%s", s);
		fprintf(fp, "%s\n", s);
		dealloc(s);
