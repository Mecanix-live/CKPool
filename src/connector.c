...
static void redirect_client(ckpool_t *ckp, client_instance_t *client)
{
    ...
    cdata_t *cdata = ckp->cdata;
    json_t *val;
    char *buf, *buf_with_eol;
    size_t len;
    int num;

    /* Set the redirected bool to only try redirecting them once */
    ...
    if (unlikely(!buf)) {
        LOGWARNING("Failed to create JSON dump in redirect_client");
        return;
    }

    len = strlen(buf);
    buf_with_eol = realloc(buf, len + 2);
    if (unlikely(!buf_with_eol)) {
        free(buf);
        LOGWARNING("Memory allocation failed in redirect_client");
        return;
    }
    buf = buf_with_eol;
    strcat(buf, "\n");
...
static void send_client_json(ckpool_t *ckp, cdata_t *cdata, int64_t client_id, json_t *json_msg)
{
    ...
    if (unlikely(!msg)) {
        LOGWARNING("Failed to create JSON dump in send_client_json");
    ...
    if (unlikely(!msg_with_eol)) {
        free(msg);
        LOGWARNING("Memory allocation failed in send_client_json");
        json_decref(json_msg);
        return;
    }
    msg = msg_with_eol;
