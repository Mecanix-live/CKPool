...
static void add_json_msgq(cs_msg_t **csmsgq, proxy_instance_t *proxy, json_t **val)
{
    ...
    // First dump JSON without EOL
    json_str = json_dumps(*val, JSON_ESCAPE_SLASH);
    json_decref(*val);
    *val = NULL;
    ...
    len = strlen(json_str);
    csmsg->buf = realloc(json_str, len + 2);
    if (unlikely(!csmsg->buf)) {
        LOGWARNING("Failed to realloc json string in add_json_msgq");
        free(json_str);
        free(csmsg);
        return;
    }

    // Append newline and update length
    strcat(csmsg->buf, "\n");
    csmsg->len = len + 1;
    csmsg->proxy = proxy;
...
void generator_add_send(ckpool_t *ckp, json_t *val)
{
    ...
    // First generate JSON without EOL
    ...
    if (unlikely(!buf_with_eol)) {
        free(buf);
        LOGWARNING("Memory allocation failed in generator_add_send");
        goto out;
    }
    buf = buf_with_eol;
    strcat(buf, "\n");
